...
class MaoyanController(PlatformGrabber):
    ...
    async def _grab_loop_blind(self) -> dict:
        """盲点模式：零 UI dump，纯坐标点击，Phase 2+3 总耗时 ~200ms。"""
        sess = self.session
        bx, by = self.buy_pos
        t0 = time.time()
        TAP_GAP = 0.05  # 50ms between taps
        ...
        # ── Phase 2: 盲点选票价 → 选观演人 → 提交，每步 50ms ──
        logger.info("[盲点] Phase 2: 开始盲点序列")
        phase2_start = time.time()

        # 连续执行 3 轮盲点序列，确保页面加载后至少有一轮命中
        for wave in range(3):
            if self.price_pos:
    ...
    async def _grab_loop_scan(self) -> dict:
        """扫描模式：通过持久 session UI dump 查找按钮，作为无校准时的 fallback。"""
        sess = self.session
        price = str(self.cfg.get("price", ""))
        viewer = self.cfg.get("viewer_name", "")
        bx, by = self.buy_pos
    ...
    async def warmup(self) -> None:
        ...
        if self.buy_pos:
            if self._blind_mode:
                ...
                logger.info("猫眼预热完成 — 扫描模式（未校准，建议运行 python tools/maoyan_calibrate.py）")
        else:
    ...
    async def grab(self) -> dict:
        ...
        try:
            if self._blind_mode:
                ...
                return await asyncio.wait_for(self._grab_loop_blind(), timeout=120)
            else:
                logger.info("=== 猫眼: 使用扫描模式 ===")
                return await asyncio.wait_for(self._grab_loop_scan(), timeout=120)
        finally:
            if self.session:
